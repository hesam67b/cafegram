...
import com.negahetazehco.cafetelegram.DialogAlert;
...
public class LocaleController {
    ...
    public static class FontInfo {
        ...
        public static FontInfo createWithString(String string) {
            if (string == null || string.length() == 0) {
                return null;
            }
            String[] args = string.split("\\|");
            ...
            FontInfo localeInfo = new FontInfo();
            localeInfo.name = args[0];
            localeInfo.value = args[1];
            return localeInfo;
        }
    }

    public ArrayList<FontInfo> sortedFonts = new ArrayList<>();
    ...
    public LocaleController() {
        ...
        fontInfo.name = "پیش فرض";
        fontInfo.value = "rmedium.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "ایران سنس معمولی";
        fontInfo.value = "iransans.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "دیپلمات";
        fontInfo.value = "rdiplomat.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "ایران سنس متوسط";
        fontInfo.value = "iransans_medium.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "ایران سنس بزرگ";
        fontInfo.value = "iransans_bold.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "دست نویس";
        fontInfo.value = "rfar_dastnevis.otf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "کتاب";
        fontInfo.value = "rketab.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "نازنین";
        fontInfo.value = "rnazanin.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "هـما";
        fontInfo.value = "rhoma.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "یکـان";
        fontInfo.value = "ryekan.ttf";
        sortedFonts.add(fontInfo);

        fontInfo = new FontInfo();
        fontInfo.name = "کودک بزرگ";
        fontInfo.value = "rkoodkbd.ttf";
        sortedFonts.add(fontInfo);
    }
    ...
    public void applyFont(final FontInfo fontInfo, final Context context) {
        String font = fontInfo.value.toString();
        if (!Setting.get(Setting.FONT_FAMILY_LABLE, Setting.FONT_FAMILY_VALUE).equals(font)) {
            Setting.set(Setting.FONT_FAMILY_LABLE, font);

            final DialogAlert dialog = new DialogAlert(context, R.style.DialogAnimation);
            dialog.setCancelable(false);
            dialog.txtDesc.setText(LocaleController.getString("FontFamilyDesc", R.string.FontFamilyDesc));
    ...
    private FastDateFormat createFormatter(Locale locale, String format, String defaultFormat) {
        ...
        if(Setting.get(Setting.DATE_FARSI_STATUS_LABLE, Setting.DATE_FARSI_STATUS_VALUE)) {
            try {
